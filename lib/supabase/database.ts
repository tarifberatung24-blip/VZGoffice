...
export type CaseStatus = 'IN_PROCESS' | 'NEEDS_USER' | 'DRAFT_READY' | 'AWAITING_APPROVAL' | 'ARCHIVED' | 'NEW' | 'UPLOADED' | 'EXTRACTING' | 'NEEDS_INFO' | 'DRAFTING' | 'NEEDS_CONFIRMATION' | 'APPROVED' | 'EXPORTED' | 'SENT' | 'WAITING_REPLY' | 'ACTION_REQUIRED' | 'CLOSED' | 'FAILED_RETRYABLE' | 'FAILED_FINAL' | 'HUMAN_REVIEW'
...
export type Database = { public: { Tables: {
  ...
  source_documents: Row<{ id: string; owner_id: string; case_id: string; path: string; mime_type: 'application/pdf' | 'image/jpeg' | 'image/png'; bytes: number; sha256: string; status: 'UPLOADED' | 'OCR_READY' | 'OCR_FAILED' | 'DELETED'; created_at: string }>
  ...
  tasks: Row<{ id: string; owner_id: string; case_id: string; type: 'reminder' | 'human_review'; due_at: string | null; status: 'open' | 'done' | 'cancelled'; idempotency_key: string; created_at: string }>
